...
spells = { // creating a function literally called spells
    WOOP: function(){ // woop is just the name of this spell
        player.move(randomPassableTile()); // moves us to a random tile
    }, // always add a comma to add new spells
    QUAKE: function(){                  
    ...
    AURA: function(){
        player.tile.getAdjacentNeighbors().forEach(function(t){
            t.setEffect(13); // this is the spritesheet spot
            if(t.monster){
                t.monster.heal(1);
            }
        });
